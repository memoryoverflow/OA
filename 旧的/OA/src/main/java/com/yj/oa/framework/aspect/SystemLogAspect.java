...
import com.yj.oa.project.po.Log;
...
public class SystemLogAspect{
    ...
    public void HandelLog(JoinPoint joinPoint, HttpServletRequest request, Throwable e)
    {
        ...
        log.setMethod(className + "/" + methodName + "()" + "/" + type);
        log.setReqType(type);

        //请求参数
        Map<String, String[]> params = request.getParameterMap();

        String mapToParams = operLog.getMapToParams(params);
        operLog.setParams(mapToParams);
        log.setParams(mapToParams);


        operLog.setStatus(OperLogStatusEnum.SUCCESS_STATUS.getValue());
        operLog.setType(OperLogTypeEnum.SUCCESS_TYPE.getValue());
        log.setType(OperLogTypeEnum.SUCCESS_TYPE.getValue());
        ...
        // rabbitTemplate.convertAndSend(JSON.toJSONString(log));
    }
