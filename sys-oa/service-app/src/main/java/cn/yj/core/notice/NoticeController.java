...
public class NoticeController extends AbstractController<Notice>
{
    ...
    public ResponseEntity downloadEnclosure(@RequestParam String url) throws IOException
    {
        // 将文件下载到该目录
        ...
        String fileDownThisPath = userHome;
        FileUtils.createFolder(fileDownThisPath);
        // 新建一个说明文件
        String remarkFile = fileDownThisPath.concat("/").concat("备注说明.txt");
        ...
        try
        {
            ...
            IOUtils.copy(inputStream, new FileOutputStream(fileDownThisPath.concat("/").concat(fileName)));
        ...
        ZipUtils.toZip(fileDownThisPath, new FileOutputStream(zipFile), true);
        // 删除掉临时文件 和文件夹
        ...
        AppExecutor.exec(() -> {
            ...
            FileUtils.delete(fileDownThisPath);
        });
        ...
        return new ResponseEntity<>(
                org.apache.commons.io.FileUtils.readFileToByteArray(new File(zipFile)),
